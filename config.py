...
SCROLL_RESET_EVERY_MIN = 30     # 刷新间隔随机下限
SCROLL_RESET_EVERY_MAX = 55     # 刷新间隔随机上限
SCRAPE_BURST_SECONDS = 50       # 每次连续抓取时长（秒），之后休息模拟用户离开
SCRAPE_IDLE_SECONDS = 45        # 休息时长（秒），之后继续下一轮 burst
...
# --- Worker 触发 Agent ---
AGENT_AUTO_TRIGGER = True         # 是否在 worker 每轮结束后自动触发 Agent
AGENT_TRIGGER_INTERVAL = 3        # 每 N 轮触发一次（1=每轮, 3=隔两轮）
AGENT_HERMES_JOB_ID = "a53a7fc71ebf"  # Hermes cron job ID
