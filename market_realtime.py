...
def _get_position_tokens() -> list[str]:
    try:
        with storage.get_conn() as conn:
            return sorted(
                p["token"].upper() for p in storage.trade_open_positions(conn) if p["token"]
            )
    except Exception:
        return []
...
def _streams_for(tokens: list[str], light_tokens: list[str] | None = None) -> list[str]:
    light_tokens = (light_tokens or [])
    depth_interval = getattr(config, "REALTIME_DEPTH_INTERVAL", "500ms")
    use_light = getattr(config, "REALTIME_WATCHLIST_LIGHT", True)

    streams = []
    position_set = {t.upper() for t in tokens}
    light_set = {t.upper() for t in light_tokens}

    for token in sorted(set(list(tokens) + list(light_tokens))):
        sym = _symbol(token).lower()
        is_position = token.upper() in position_set
        is_light = token.upper() in light_set and not is_position and use_light
...
async def run_for_watchlist(position_tokens: list[str],
                            watchlist_tokens: list[str]):
    token_set = {t.upper() for t in position_tokens + watchlist_tokens}
    streams = _streams_for(position_tokens, watchlist_tokens)
    if not streams:
        return

    url = FSTREAM_BASE + "/".join(streams)
    state = RealtimeState(position_tokens + watchlist_tokens)
    last_flush = 0.0
    pos_count = len(position_tokens)
    wl_count = len(set(watchlist_tokens) - set(position_tokens))
    console.print(
        f"[green]实时行情订阅: {pos_count} 持仓 + {wl_count} 观察[/green]"
        f" ([dim]{len(streams)} streams[/dim])"
    )
    ...
    async with websockets.connect(url, ping_interval=20, ping_timeout=20) as ws:
        ...
        while _running:
            ...
            try:
                ...
                console.print(f"[yellow]WS 连接断开 ({e})，稍后重连...[/yellow]")
                break
            except Exception as e:
...
async def main():
    ...
    while _running:
        pos_tokens = _get_position_tokens()
        all_tokens = _get_realtime_tokens()
        if not all_tokens:
            console.print("[yellow]实时订阅列表为空，等待添加观察或持仓代币...[/yellow]")
            await asyncio.sleep(config.REALTIME_WATCHLIST_POLL_SECONDS)
            continue
        wl_tokens = [t for t in all_tokens if t not in set(pos_tokens)]
        try:
            await run_for_watchlist(pos_tokens, wl_tokens)
        except Exception as e:
            if _running:
