...
### positions 的 token 字段被掩码为 `***`

`extract_market_data.py` 输出的 `positions` 里，`token` 字段的值全部是 `"***"`，JSON dump 和 Python `.get('token')` 都只返回 `"***"`。**但 `today_journal` 的 token 是真实值，`candidates` 里的 token 也是真实值。**

> ⚠️ `today_journal` 的 `action_type` 字段在实践中为 `None`，不能用它区分开仓/平仓/止损。通过 reason 文本判断：含「止损」「止盈」为退出，含「Taker」「共振」「买盘信号」等为开仓。

**如何识别持仓对应币种**（两步法，journal token 真实）：
