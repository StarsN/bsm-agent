...
`positions` 的 `token` 被掩码为 `***`。通过 `today_journal` 的 `entry_price` 匹配 journal 的 `price` 来识别真实币种。`today_journal` 通过 `reason` 文本判断开仓/平仓（含"止损""止盈"为退出）。详见 `references/数据操作要点.md`。
...
- **`agent_lessons`**（复盘提炼的规律）→ 回答"这个场景犯过什么错"。每条有 `signal_error`（哪判断错了）、`root_cause`（根因）、`rule_update`（衍生的规则）。看看同 token 有没有踩过坑？全局教训（token='*'）适用吗？
- **`tag_stats + archive_lessons`**（系统自动打的失败标签统计）→ 回答"什么错最常犯"。看看当前候选是否命中高频失败标签（如 `buy_pressure_faded`）。

如果当前场景和某条教训吻合，要么不开，要么在 reason 里说明为什么这次不同。详见 `references/lessons字段说明.md` 和 `references/失败的教训标签.md`。
