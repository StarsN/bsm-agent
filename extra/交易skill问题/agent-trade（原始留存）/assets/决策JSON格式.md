  ...
  "decisions": [
    {
      ...
      "social_score": 12.5,
      "mentions": 3,
      "dimension_data": "JSON.stringify 候选对象的顶层市场字段（price/15m/1h/oi_1h/taker/funding 等），写英文 key，系统自动翻译中文入库",
...
| reason | **必填**，详细决策理由 |
| source_round | 从提取数据的 `current_round` 取 |
| social_score | 从对应候选的 `social_score` 取 |
| mentions | 从对应候选的 `mentions` 取 |
| dimension_data | 入场时的市场数据快照，用候选对象顶层英文 key 构造 JSON。系统自动翻译中文入库 |
