...
⚠️ **`c["analysis"]` 可能全为 `None`**：当 worker 处于 `stage=scraping`（数据采集阶段）时，分析模块尚未运行，`verdict`/`direction`/`tags`/`oi_divergence` 等字段均可能为 `None`。此时必须完全依赖原始市场数据自行判断，不可因分析缺失而跳过分析环节。
...
- **可运行辅助脚本**：`python3 scripts/detect_traps.py` 自动扫描所有候选币，标记 4 大陷阱（T1 买压消退/T2 OI背离/T3 高费率追高/T4 薄盘口）+ 2 条 agent 教训（追高阈值/派发模式），加速筛选。
...
> ⚠️ **常见 reject 原因**：缺必填字段（action/token/tier/reason/source_round/social_score/mentions/dimension_data/market_overview/lesson_checked，缺一不可）、action 写成 `"side": "LONG"` 而非 `"open_long"`、dimension_data 未序列化为 JSON 字符串。被 reject 后必须补全所有字段重试。
