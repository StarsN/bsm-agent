...
# ---- 候选币（从 token_heat_history 取最近 3 轮）----
...
for h in candidates_raw:
    ...
    if snap:
        ...
        candidates.append({
            ...
            "tags": a.get("tags", []),
            "notes": a.get("notes", []),
            # 上币时长估算（基于 15m K 线实际数量，100 根 = 至少 25h）
            "age": (
                None if not s.get("klines_15m_count") else
                ">1d" if s["klines_15m_count"] >= 96 else
                f"{s['klines_15m_count'] * 15 // 60 // 24}d{s['klines_15m_count'] * 15 // 60 % 24}h"
            ),
        })
...
print(f"  候选: {len(candidates)} 有快照 / {len(rows)} 总行数(3轮)")
