---
name: agent-review-heat-lessons
...
```bash
python3 /root/binance-monitor/bsm-agent/agent-review_heat_lessons/scripts/extract_review_data.py --output /tmp/review_data_heat_lessons.json
```
...
2、动能反转 (Taker Delta)：`主动买卖比` 从入场强势（>1.2）跌破 1.0 变为卖盘主导？同时对比 `Taker趋势` 的衰减幅度——趋势从正转负且幅度大说明买盘彻底退潮。（买盘资金真实撤退）。
3、聪明钱倒戈 (Top Trader LSR Delta)：`大户多空比` 是否发生方向性反转？（大资金离场）。
4、流动性击穿 (Depth Delta)：`买盘深度(USD)` 是否枯竭，伴随 `盘口价差` 放大？（遭遇流动性陷阱）。
5、入场动能质量 (Entry Taker Trend)：`Taker趋势` 在入场时本身就是一个信号质量指标——将全部交易的入场 Taker趋势值按盈亏分组，寻找规律。详见 `references/Taker趋势风险曲线.md`。
   - 趋势<+15：买盘动能停滞，快照Taker值可能是残影而非持续力量。即使快照达标（>1.15），持仓也极易因买盘衰竭而止损。
   - 趋势>+200：极端高值往往是FOMO末期的耗尽信号而非"超强动能"——大量买家已集中入场，后续缺乏增量。叠加15m OI下降时更危险。
   - 趋势在+25~+200之间：健康加速区间，是盈利单集中的区域。
   这是 U 形风险曲线——两端（过低/过高）都危险，中间才是真正的动能。
发现超出以上五类的新模式（资金费率骤变、散户大户背离等），记录并引用具体字段和数值。
...
```bash
python3 /root/binance-monitor/bsm-agent/agent-review_heat_lessons/scripts/write_lessons.py --lessons /tmp/agent_lessons_heat_lessons.json --strategy heat_agent_lessons --journal-ids "从提取数据中的journal_ids字段取，逗号拼接，无论这笔单子是否生成了教训，都必须传全量 ID，以标记整批数据已处理完，防止死循环！无则传 NONE"
```
