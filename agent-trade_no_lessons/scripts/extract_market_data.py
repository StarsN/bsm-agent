...
# ---- 候选币（nl_candidates 在 nl.db）----
inter_min = 15
try:
    ts = {r["key"]: r["value"] for r in conn.execute("SELECT * FROM trading_settings").fetchall()}
    inter_min = int(ts.get("nl_agent_interval_minutes", getattr(config, "NL_AGENT_INTERVAL_MINUTES", 30)))
except Exception:
    pass
ac_rows = []
try:
    _agent_db = str(PROJECT_DIR / getattr(config, "NL_DB", "db/nl.db"))
    _agent_conn = sqlite3.connect(_agent_db)
    _agent_conn.row_factory = sqlite3.Row
    ac_rows = _agent_conn.execute(
        f"SELECT a.data FROM nl_candidates a "
        "INNER JOIN ("
        "  SELECT token, MAX(id) AS max_id FROM nl_candidates "
...
initial = float(settings.get("strategy_initial_agent_no_lessons", 1000))
realized = conn.execute(
    "SELECT COALESCE(SUM(realized_pnl),0) FROM trade_positions WHERE strategy='agent_no_lessons'"
).fetchone()[0]
unrealized = conn.execute(
    "SELECT COALESCE(SUM(unrealized_pnl),0) FROM trade_positions "
    "WHERE status IN ('OPEN','PARTIAL') AND strategy='agent_no_lessons'"
).fetchone()[0]
locked = conn.execute(
...
today_count = conn.execute(
    ...
    "WHERE strategy='agent_no_lessons' AND date(created_at, '+8 hours') = date('now', '+8 hours')"
).fetchone()[0]
...
for r in conn.execute(
    """SELECT la.token, la.pnl_pct, la.failed_reason, la.reason_tags
       ...
       WHERE tp.strategy = 'agent_no_lessons'
...
for r in conn.execute(
    """SELECT la.reason_tags FROM trade_loss_archive la
       JOIN trade_positions tp ON la.position_id = tp.id
       WHERE la.reason_tags IS NOT NULL AND tp.strategy = 'agent_no_lessons'"""
):
...
today_journal = [dict(r) for r in conn.execute(
    ...
    "AND tp.strategy = 'agent_no_lessons' "
    "ORDER BY j.id"
)]
